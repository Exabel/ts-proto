...
function generateSimpleEnum(options, fullName, enumDesc, sourceInfo) {
    let code = ts_poet_1.CodeBlock.empty();
    utils_1.maybeAddComment(sourceInfo, (text) => (code = code.add(`/**\n * %L */\n`, text)));
        ...
        const readSnippet = (from) => {
            if (types_1.isEnum(field)) {
                return types_1.isWithinOneOf(field)
                    ? ts_poet_1.CodeBlock.of('%L !== undefined ? %T.toJSON(%L) : undefined', from, types_1.basicTypeName(typeMap, field, options), from)
                    : ts_poet_1.CodeBlock.of('%T.toJSON(%L)', types_1.basicTypeName(typeMap, field, options), from);
            }
            else if (types_1.isTimestamp(field)) {
